...
def plot_scatter_durations(in_Symbol,x,y,in_titles,in_color,in_config,xlim,ylim) :
    ...
    plt.title(in_Symbol + '_' + str(round(correlation[0,1],2)) ,fontsize = 7)
...
def plot_scatter_durations_subplot(in_Symbol,toPlot,in_nRowsTotalSubPlot, in_nColsTotalSubPlot, in_plot_count,in_titles,in_color,in_config,xlim,ylim):
    ...
    plot_scatter_durations(in_Symbol,x,y,in_titles,in_color,in_config,xlim,ylim)
...
def plt_overwrite_and_save(strFile):
...
def get_col_row_correlation_graph(in_toDisplay,in_configcommon) :
    ...
    return (in_nRowsTotalSubPlot,in_nColsTotalSubPlot)
