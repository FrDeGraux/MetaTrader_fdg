...
class Reporter:
  def __init__(self,in_config_specific,in_config_subrun,in_config_common):
      ...
      self.df_positions = self.df_positions.head(5000)
  ...
  def compute_equity(self,in_symbol) :
      ...
      positions_cumulative = positions_cumulative.drop(columns=['PL_Points','PL_Points_Euro', 'Point_value','Symbol'])    #2
      positions_cumulative.index = pd.to_datetime(positions_cumulative.index)
