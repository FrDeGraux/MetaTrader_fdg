...
class Reporter:
  ...
  def plot_MAE_MFE_bySymbol(self,item,data,count,in_nRowsTotalSubPlot_MAE_MFE,in_nColsTotalSubPlot_MAE_MFE,xlim):
          ...
          plt.scatter(profit, MAE, s=int(self.configcommon.get('Correlations', 'Correlations_Marker_Size')),label=('MAE' + item + '_'))
          plt.scatter(profit,MFE, s=int(self.configcommon.get('Correlations', 'Correlations_Marker_Size')),label=('MFE' + item + '_'))
  ...
  def plot_return_durations_all_symbols(self):
      profits_duration = {}
      [profits_duration.update({item : compute_profit_durations(self.df_positions,item,self.configcommon)}) for item in self.symbolList]
      ...
      for item in list(profits_duration.values()) : # list of tuple
      ...
      [plot_scatter_durations_subplot(item,profits_duration[item],in_nColsTotalSubPlot,in_nRowsTotalSubPlot,count+1,titles,'b',self.configcommon,xlim_symbols,ylim_symbols) for count,item in enumerate(self.symbolListNoALL)]
