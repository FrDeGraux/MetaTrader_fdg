...
from utilData import filterBySymbol,getSymbolList
...
def scatter_data(in_df,in_Symbol,min_hyst,max_hyst,z_min,z_max):
    ...
    ax.set_xlabel('Hysteresis_in')
    ax.set_ylabel('Hysteresis_out')
...
def compute_hyper_cube(in_df,in_theshold,min_step,max_step,nSteps = 100) :
    ...
    for item in sample :
        ...
        if( duration_item > in_theshold) :
            return (item,minimum_time[-1],maximum_time[-1])
            # return max of

    return (sample[-1],minimum_time[-1],maximum_time[-1])
def build_all_hyper_cubes(in_df,in_symbol,in_threshold,in_cfg) :


    dtStart = datetime.date(2012, 1, 1)
    dtEnd = datetime.date(2022, 1, 1)
    d = datetime.date(2020, 2, 29)
    v = d + relativedelta(years=1)

    dt_min = in_df['TimeIn'].min()
    dt_max = in_df['TimeIn'].max()
    ...
    for i in range(diff.years):
        ...
        filtered_df = in_df[ (in_df['TimeIn'] >= startTime)]
        filtered_df = filtered_df[(in_df['TimeIn'] < (startTime+ relativedelta(years=1)))]

        build_hyper_cube(filtered_df,startTime,startTime+ relativedelta(years=1),in_symbol,in_threshold,in_cfg)

def build_hyper_cube(in_df,dtStart,dtEnd,in_symbol,in_threshold,in_cfg) :
    ...
    in_df['duration'] = in_df['TimeOut'] - in_df['TimeIn']
    if(in_threshold > 1 or in_threshold < 0) :
    ...
    in_df['duration'] = in_df.apply(lambda row: tot_hours(row['duration']), axis=1)
    ...
    max_hyst,z_min,z_max = compute_hyper_cube(in_df,in_threshold,min_hyst,max_hyst)
    ax = scatter_data(in_df, in_symbol, min_hyst, max_hyst, z_min, z_max)
    ax.set_title(in_symbol + " Fixed hysteresis calibration " + "(" + dtStart.strftime("%Y") + ";" + dtEnd.strftime("%Y"))
