...
public:
                     CGuruEx03_Base();
                     CGuruEx03_Base(int _magicNumber,int _ATR_MAPeriod);               // Constructor
...
CGuruEx03_Base::CGuruEx03_Base(int _magicNumber,int _ATR_MAPeriod)
  {
  ticks = 0;
   ...
   int factor = 0;
    ...
    return(factor*(price-3*atr_value));
   ...
   int factor = 0;
     ...
     return(factor*(price+6*atr_value));
   }
//+------------------------------------------------------------------+
//| Performs system initialisation                                   |
//+------------------------------------------------------------------+
bool CGuruEx03_Base::Init(string Pair,int slippage,double lot)
  {
 ...
 if(m_ATR == NULL)
     {
      if((m_ATR = new CiATR) == NULL)
        {
         Print("Error creating CiRAVI");
         return(false);
        }
     }
   if(!m_ATR.Create(m_Pair, 0,ATR_MAPeriod))
     {
      Print("Error initializing CiADX");
      return(false);
     }
